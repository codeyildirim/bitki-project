...
const AdminProducts = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (editingProduct) {
        response = await adminApi.put(`/products/${editingProduct.id}`, formDataObj, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      } else {
        response = await adminApi.post('/products', formDataObj, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      }
  ...
  const toggleFeatured = async (productId) => {
    try {
      const response = await adminApi.patch(`/products/${productId}/featured`);
      if (response.data.success) {
        loadProducts();
